...
def proxy_handler(client_socket, remote_host, remote_port, recvive_first):
    ...
    if len(remote_buffer):
        line = "[<==] Sending %d bytes from localhost." %len(remote_buffer)
...
def server_loop(local_host, local_port, remote_host, remote_port, receive_first):
    ...
    while True:
        ...
        line = "> Receive incoming connection from %s:%d" %(addr[0], addr[1])
        print(line)
...
def main():
    if len(sys.argv[1:]) != 5:
        ...
        print("Example: ./TCP_Proxy.py 127.0.0.1 9000 10.12.11.3 9000 True")
        sys.exit(0)
